...
newoption {
	trigger = "zig-target",
	description = "target flag for zig",
}
...
--
	...
	premake.zig.cc     = "zig cc"
	premake.zig.cxx    = "zig c++"
	premake.zig.ar     = "zig ar"
	premake.zig.rc     = "windres"
	premake.zig.llvm   = true

	if _OPTIONS["zig-target"] then
		premake.zig.cc  = string.format("%s -target %s", premake.zig.cc, _OPTIONS["zig-target"])
		premake.zig.cxx = string.format("%s -target %s", premake.zig.cxx, _OPTIONS["zig-target"])
	else
		premake.zig.cc  = string.format("%s -target $ZIGTARGET", premake.zig.cc)
		premake.zig.cxx = string.format("%s -target $ZIGTARGET", premake.zig.cxx)
	end
...
--
  ...
  include $(DEVKITPPC)/wii_rules']],
	...
	local platforms = premake.zig.platforms
...
--
	...
	function premake.zig.getcppflags(cfg)
		local flags = { }
	...
	function premake.zig.getcflags(cfg)
	...
	function premake.zig.getcxxflags(cfg)
		local result = table.translate(cfg.flags, cxxflags)
		return result
	end


	function premake.zig.getobjcflags(cfg)
...
--
	...
	function premake.zig.getldflags(cfg)
		local result = { }
...
--
	...
	function premake.zig.getlibdirflags(cfg)
		local result = { }
...
--  p: path
--
	function premake.zig.islibfile(cfg, p)
...
--  cfg: configuration
--
	function premake.zig.getlibfiles(cfg)
		local result = {}
		for _, value in ipairs(premake.getlinks(cfg, "system", "fullpath")) do
			if premake.zig.islibfile(cfg, value) then
...
--
	...
	function premake.zig.getlinkflags(cfg)
		local result = {}
		for _, value in ipairs(premake.getlinks(cfg, "system", "fullpath")) do
			if premake.zig.islibfile(cfg, value) and path.getdirectory(value) ~= "." then
...
--
	...
	function premake.zig.wholearchive(lib)
		if premake.zig.llvm then
...
--
	...
	function premake.zig.getarchiveflags(prj, cfg, ndx)
		local result = {}
...
--
	...
	function premake.zig.getdefines(defines)
		local result = { }
...
--
	...
	function premake.zig.getincludedirs(includedirs)
		local result = { }
...
--
	...
	function premake.zig.getquoteincludedirs(includedirs)
		local result = { }
		for _,dir in ipairs(includedirs) do
...
--
	...
	function premake.zig.getsystemincludedirs(includedirs)
		local result = { }
		for _,dir in ipairs(includedirs) do
...
--
	...
	function premake.zig.getcfgsettings(cfg)
